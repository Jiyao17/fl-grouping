...
sub_dirs = ["grouping/cvg_cvs/"] * 4 + ["grouping/qcid/"]
...
# marks = ["_rgr1_rgr", "_rgr10_rgr", "_rgr50_rgr", "_rgr1000_rgr"] # 300
marks = ["_rgr1_100_esrcv", "_rgr10_100_esrcv", "_rgr50_100_esrcv", "_rgr1000_100_esrcv", "_100"] # 100
# marks = ["_rgr1_200_esrcv", "_rgr10_200_esrcv", "_rgr50_200_esrcv", "_rgr1000_200_esrcv", "_200"] # 200
# marks = ["_rgr1_300_esrcv", "_rgr10_300_esrcv", "_rgr50_300_esrcv", "_rgr1000_300_esrcv", "_300"] # 300
# marks = ["_rgr1_rgr1", "_rgr10_rgr1", "_rgr50_rgr1", "_rgr1000_rgr1"] # 100

# marks = ["_rgr1_rgr_rcv", "_rgr10_rgr_rcv", "_rgr50_rgr_rcv", "_rgr1000_rgr_rcv"] # 300
# marks = ["_rgr1_100_rcv", "_rgr10_100_rcv", "_rgr50_100_rcv", "_rgr1000_100_rcv", ""] # 100
# marks = ["_rgr1_100_srcv", "_rgr10_100_srcv", "_rgr50_100_srcv", "_rgr1000_100_srcv"] # 300
# marks = ["_rgr1_300_srcv", "_rgr10_300_srcv", "_rgr50_300_srcv", "_rgr1000_300_srcv"] # 100
#
...
colors = [ colors[3], colors[0], colors[1], colors[2], "black" ]
fig_labels = ["1", "10", "50", "Inf", "QCID"]
styles = ["-", "-", "-", "-", "--"]

lines = [-1, -1, -1, -1, -3]
for sub_dir, mark, label, line, style, color in zip(sub_dirs, marks, fig_labels, lines, styles, colors):
    cost_filename = root_data_dir + sub_dir + "cost" + mark
    accu_filename = root_data_dir + sub_dir + "accu" + mark

    costs, accus = [], []
    min_len = np.inf
    cost = open(cost_filename, "r").readlines()[line].strip().split()
    if len(cost) > 0 and cost[0] != "" and cost[0][0] == "c":
        continue
    cost = [float(x) for x in cost if x != ""]
    accu = open(accu_filename, "r").readlines()[line].strip().split()
    ...
    cost = np.zeros(min_len)
    accu = np.zeros(min_len)
    for c in costs:
        cost += np.array(c[:min_len])
    for a in accus:
        accu += np.array(a[:min_len])
    ...
    print(label)
    print(max_accu)
    plt.plot(cost, accu, label=label, linestyle=style, color=color)
...
plt.subplots_adjust(0.18, 0.16, 0.94, 0.96)
...
plt.savefig("comp_rgr_qcid.png")
plt.savefig("comp_rgr_qcid.pdf")

...
# marks = ["_alpha0.1_cv1.0_5*2", "_alpha0.1_cv0.5_5*2", "_alpha0.1_cv0.1_5*2" ]
...
# comp_CV
marks = ["_alpha0.1_cv0.5_10*2", "_alpha0.1_cv1.0_5*2", "_alpha0.1_gs5_5*1", "_alpha0.1_gs10_5*1", "_alpha0.1_gs15_5*1", ]
fig_labels = ["CoVG(0.5) AvgGS=6.13", "CoVG(1.0) AvgGS=5.03", "RG GS=5", "RG GS=10", "RG GS=15",]


marks = ["_gs5_rg_alpha0.1_gs5_10*2", "_gs10_rg_alpha0.1_gs10_10*2", "_gs20_rg_alpha0.1_gs20_10*2", "_gs5_cvg_alpha0.1_gs5_10*2", "_gs10_cvg_alpha0.1_gs10_10*2","_gs20_cvg_alpha0.1_gs20_10*2", ]
fig_labels = ["RG GS=5", "RG GS=10", "RG GS=20", "CoVG GS=5", "CoVG GS=10", "CoVG GS=20",]
colors = [ colors[0], colors[1], colors[2], colors[0], colors[1], colors[2] ]
styles = [ "--", "--", "--", "-", "-", "-",  ]
sub_dirs = ["debug/"] * 3
marks = ["_gs5_rg_alpha0.1_gs5_10*2", "_gs10_rg_alpha0.1_gs10_10*2",  "_gs20_rg_alpha0.1_gs20_10*2", ]
# fig_labels = ["RG GS=5", "RG GS=10", "CoVG GS=5", "CoVG GS=10", ]
fig_labels = ["RG GS=5", "RG GS=10", "RG GS=20", ]
...
# comp audio
# marks = ["_alpha0.1_cv1.0_5*2", "_unbiased_esrcv_alpha0.1_gs5_5*2", "_biased_srcv_alpha0.1_gs5_5*2", "_unbiased_srcv_alpha0.1_gs5_5*2", ]
# fig_labels = ["ESRCV-Biased", "ESRCV-Unbiased", "SRCV-Biased", "SRCV-Unbiased"]
...
a = 1
test_num = 3
for sub_dir, mark, label, color, style in zip(sub_dirs, marks, fig_labels, colors, styles):
    # if a % 2 == 0:
    #     a += 1
    #     # continue
    # a += 1
    ...
    # avg_cost = [ cost[i] for i in range(len(cost)) ]
    # avg_accu = [ accu[i] for i in range(len(accu)) ]
    acc_cost = []
    acc_accu = []
    for i in range(0 , test_num):
        ...
        if len(acc_cost) == 0:
            acc_cost = [ cost[i] for i in range(len(cost)) ]
            acc_accu = [ accu[i] for i in range(len(accu)) ]
        else:
            acc_cost = [ acc_cost[i] + cost[i] for i in range(len(acc_cost))]
            acc_accu = [ acc_accu[i] + accu[i] for i in range(len(acc_accu)) ]
        
    avg_cost = [ acc_cost[i] / test_num for i in range(len(acc_cost)) ] 
    avg_accu = [ acc_accu[i] / test_num for i in range(len(acc_accu)) ]    
    ...
    # plt.plot(avg_cost, avg_accu, label=label, color=color, linestyle=style)
    plt.plot(range(len(avg_cost)), avg_accu,  label=label, color=color, linestyle=style)
...
plt.savefig("accu_by_iter.png")
plt.savefig("accu_by_iter.pdf")

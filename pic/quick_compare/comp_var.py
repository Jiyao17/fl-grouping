...
# comp audio
marks = ["_cvg_esrcv_alpha0.1_gs5_10*2","_cvg_srcv_alpha0.1_gs5_10*2", "_cvg_rcv_alpha0.1_gs5_10*2" ]
fig_labels = ["ESRCoV", "SRCoV","RCoV",  ]
...
for sub_dir, mark, label in zip(sub_dirs, marks, fig_labels):
    ...
    cost = open(cost_filename, "r").readlines()[-1 + correction_line].strip().split()
    if cost[0] != "" and cost[0][0] == "c":
        continue
    cost = [float(x) for x in cost if x != ""]
    accu = open(accu_filename, "r").readlines()[-1 + correction_line].strip().split()
...
plt.rc('font', size=20)
plt.subplots_adjust(0.20, 0.16, 0.95, 0.96)
...
plt.savefig("comp_var.png")
plt.savefig("comp_var.pdf")

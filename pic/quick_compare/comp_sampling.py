...
# comp audio
marks = ["_esrcv_sampling0.2_alpha0.1_gs5_5*2" ] 
fig_labels = ["ESRCV-2/10", ]
...
for sub_dir, mark, label, color, style in zip(sub_dirs, marks, fig_labels, colors, styles):
    ...
    plt.plot(cost, accu, label=label, color=color, linestyle=style)
    # plt.plot(range(len(cost)), accu,  label=label, color=color, linestyle=style)
...
plt.savefig("comp_sampling.png")
plt.savefig("comp_sampling.pdf")

...
for sub_dir, mark, label in zip(sub_dirs, marks, fig_labels):
    ...
    #     cost = cost[:41]
    #     accu = accu[:41]
    # create np array from inhomogeneous lists
    min_len = min([len(cost) for cost in costs])
    costs = [cost[:min_len] for cost in costs]
    accus = [accu[:min_len] for accu in accus]
    costs = np.array(costs)
    accus = np.array(accus)
    cost = np.mean(costs, axis=0)
    accu = np.mean(accus, axis=0)
    plt.plot(cost, accu, label=label)
    # rounds = [i*5 for i in range(len(cost))]
    # plt.plot(rounds, accu,  label=label)
...
# plt.savefig("comp_mnist.png")
plt.savefig("comp_fmnist.pdf")

...
# sub_dirs = ["grouping/rg_rs/", "grouping/ouea/", "grouping/kld/", "grouping/cvg_cvs/"]
# sub_dirs = ["grouping/rg_rs/", "grouping/ouea/", "grouping/kld/",]
sub_dirs = ["grouping/rg_rs/", "grouping/rg_cvs/",] + [ "grouping/kld/",] *2 + \
    [ "grouping/cvg_rs/", "grouping/cvg_cvs/",] 
...
# marks = ["_rgr1000",  ] * 2
marks = ["_sc_rg_rs", "_sc_rg_cvs", "_sc_kldg_rs", "_sc_kldg_cvs", "_sc_cvg_rs",
            "_sc_cvg_cvs"]
# marks = ["_sf=1.0",  ] * 4 + ["_sf=0.1",  ] * 1

for i, mark in enumerate(marks):
    # marks[i] = mark + "_cvs" + "_0"
    # marks[i] = mark + "_cvs" + "_0"
    marks[i] = mark + "_1"
    # marks[i] = mark + "_cvs" + "_a=001-01"
    # marks[i] = mark + "_rs" + "_0"
    # marks[i] = mark + "_cvs" + "_2"
...
# fig_labels = ["RG", "CDG", "KLDG", "CVG"]
fig_labels = ["RG+RS", "RG+CVS", "KLDG+RS", "KLDG+CVS", "CVG+RS", "CVG+CVS"]
styles = ["-", "--", "-", "--", "-", "--"]

lines = []
for sub_dir, mark, label, style, color in zip(sub_dirs, marks, fig_labels, styles, colors):
    ...
    # plt.plot(range(len(cost))[:num], accu[:num], label=label, linestyle=style, color=color)

    # plt.plot(cost[:num], accu[:num], label=label)
...
plt.savefig("gr_sp_comb.png")
plt.savefig("gr_sp_comb.pdf")
